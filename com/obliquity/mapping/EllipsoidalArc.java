...
public class EllipsoidalArc {
    ...
    private boolean havePositionA, havePositionB, haveAzimuthA, haveAzimuthB, haveDistance;
    ...
    public double calculateDistance() throws IllegalStateException, ArithmeticException {
	double U1, U2, w, sigma, ssig, csig, salp, c2alp, c2sigm;
	double u, A, B, C, dsig, s, b;
	...
	do {
	    ...
	    q = Math.cos(U1) * Math.sin(U2) - Math.sin(U1) * Math.cos(U2) * Math.cos(lambda);

	    ssig = Math.sqrt(p*p + q*q);

	    csig = Math.sin(U1) * Math.sin(U2) + Math.cos(U1) * Math.cos(U2) * Math.cos(lambda);
	    ...
	    dlam = (1.0 - C) * f * salp * (sigma + C * ssig *
					   (c2sigm + C * csig * (2.0 * c2sigm * c2sigm - 1.0)));
	...
	A = 1.0 + (u/16384.0) * (4096.0 + u * (-768.0 + u * (320.0 - 175.0 * u)));

	B = (u/1024.0) * (256.0 + u * (-128.0 + u * (74.0 - 47.0 * u)));

	dsig = B * ssig * (c2sigm + (B/4.0) * (csig * (2.0 * c2sigm * c2sigm - 1.0)
					       - (B/6.0) * c2sigm * (4.0 * ssig * ssig - 3.0)
	...
	q = Math.cos(U2) * Math.sin(U1) - Math.sin(U2) * Math.cos(U1) * Math.cos(lambda);
	...
	A = 1.0 + (uu/16384.0) * (4096.0 + uu * (-768.0 + uu * (320.0 -175.0 * uu)));

	B = (uu/1024.0) * (256.0 + uu * (-128.0 + uu * (74.0 - 47.0 * uu)));

	sigma1 = Math.atan(tsig1);
	if (tsig1 < 0.0) sigma1 += Math.PI;
	...
	do {
	    ...
	    dsig = B * ssig * (
			       c2sm + (B/4.0) * (
						 csig * (-1.0 + 2.0 * c2sm * c2sm)
						 - (B/6.0) * c2sm * (-3.0 + 4.0 * ssig * ssig) * (-3.0 + 4.0 * c2sm * c2sm)
						 )
			       );
